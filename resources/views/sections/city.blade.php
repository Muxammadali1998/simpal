                ...
                <div class="d-flex align-items-start">
                    <div class="nav flex-column nav-pills me-3" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                        @foreach ($cities as $key=>$city)
                                    <button style=" height: 50px; margin: 5px; border: 0.5px #009CFF solid"  class="nav-link {{ $key == 0 ? "active" : ''}} d-flex justify-content-between align-items-center" id="v-pills-home-tab{{$key}}"   data-bs-toggle="pill" data-bs-target="#v-pills-home{{$key}}" type="button" role="tab" aria-controls="v-pills-home" aria-selected="true">  {{  $city->name}}
                                         <i onclick="editmodal2('{{$city->name}}','{{ $city->id }}')" style="background-color: white;" class="btn btn-sm-square btn-outline-info m-2 bi bi-pen "></i>   </button>
